...
import com.dmcc.bid.ui.bidlist.adapter.ListAdapter;
...
//        mBaseOperation.showSweetLoading("正在搜索...");
    ...
    private void initView() {
        ...
        mSearch.setOnClickListener(v -> reSearch());
...
//        mRecyclerView.setItemAnimator(new ScaleInTopAnimator(new OvershootInterpolator(0.5f)));
        ...
        adapter.setOnItemClickListener(position -> {
            ...
            mBaseOperation.addParameter("keyword", keyword);
            if (currtParty == mISFirst) {
                mBaseOperation.addParameter("type", "firstPartys");
            } else {
                mBaseOperation.addParameter("type", "secondPartys");
            }
    ...
    private void showList(Bid bid) {
        ...
        BigDecimal percent = new BigDecimal(StringUtil.getPercent(tempCurrent, tempCount)).setScale(0, BigDecimal.ROUND_HALF_UP);
        mNumberProBar.setProgress(percent.intValue());
        ...
        mFirstCount.setDefaultText("甲方")
//                .setTextConetxt("" + (Integer.parseInt(mFirstCount.getTextContext()) + firstSize))
                .setTextColor(textColra);
        mSceondCount.setDefaultText("乙方")
//                .setTextConetxt("" + (Integer.parseInt(mSceondCount.getTextContext()) + secondSize))
                .setTextColor(textColrb);
        page++;
     ...
     *
     * @param bid
     * @param currtParty 区分甲乙方
     * @return
     */
    @NonNull
    private List<BidItem> getBidItems(Bid bid, int currtParty) {
